...
func (kc *KopiaClient) CreateOrConnectRepo(ctx context.Context, repoDir, bucketName string) error {
	...
	st, err := kc.getStorage(ctx, repoDir)
	if err != nil {
		return err
	}
...
func (kc *KopiaClient) getStorage(ctx context.Context, repoDir string) (st blob.Storage, err error) {
	//if bucketName != "" {
	if false {
		/*s3Opts := &s3.Options{
