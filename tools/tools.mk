...
# tool versions
GOLANGCI_LINT_VERSION=1.59.0 #1.22.5
...
ifeq ($(GOOS),openbsd)
	...
	go run github.com/chanhpng/vbe/tools/gettool -tool node:$(NODE_VERSION) --output-dir $(node_base_dir)
endif
...
$(linter):
	go run github.com/chanhpng/vbe/tools/gettool --tool linter:$(GOLANGCI_LINT_VERSION) --output-dir $(linter_dir)
...
$(cli2mdbin):
	go build -o $(cli2mdbin) github.com/chanhpng/vbe/tools/cli2md
...
$(hugo):
	go run github.com/chanhpng/vbe/tools/gettool --tool hugo:$(HUGO_VERSION) --output-dir $(hugo_dir)
...
$(gitchglog):
	go run github.com/chanhpng/vbe/tools/gettool --tool gitchglog:$(GITCHGLOG_VERSION) --output-dir $(gitchglog_dir)
...
$(rclone):
	go run github.com/chanhpng/vbe/tools/gettool --tool rclone:$(RCLONE_VERSION) --output-dir $(rclone_dir)
...
$(gotestsum):
	go run github.com/chanhpng/vbe/tools/gettool --tool gotestsum:$(GOTESTSUM_VERSION) --output-dir $(gotestsum_dir)
...
$(kopia08):
	go run github.com/chanhpng/vbe/tools/gettool --tool kopia:$(kopia08_version) --output-dir $(kopia08_dir)
...
$(goreleaser):
	go run github.com/chanhpng/vbe/tools/gettool --tool goreleaser:$(GORELEASER_VERSION) --output-dir $(goreleaser_dir)
...
KOPIA_BUILD_TAGS=
KOPIA_BUILD_FLAGS=-ldflags "-s -w -X github.com/chanhpng/vbe/repo.BuildVersion=$(KOPIA_VERSION_NO_PREFIX) -X github.com/chanhpng/vbe/repo.BuildInfo=$(shell git rev-parse HEAD) -X github.com/chanhpng/vbe/repo.BuildGitHubRepo=$(GITHUB_REPOSITORY)"
...
verify-all-tool-checksums:
	go run github.com/chanhpng/vbe/tools/gettool --test-all \
	  --output-dir /tmp/all-tools \
	  --tool $(ALL_TOOL_VERSIONS)

regenerate-checksums:
	go run github.com/chanhpng/vbe/tools/gettool --regenerate-checksums $(CURDIR)/tools/gettool/checksums.txt \
	  --output-dir /tmp/all-tools \
	  --tool $(ALL_TOOL_VERSIONS)
