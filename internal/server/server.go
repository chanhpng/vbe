...
import (
	...
	"github.com/chanhpng/vbe/internal/scheduler"
...
func (s *Server) ServeStaticFiles(m *mux.Router, fs http.FileSystem) {
	...
	m.PathPrefix("/").HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !requireUIUser(rc.req.Context(), rc) {
			http.Error(w, `UI Access denied. See https://github.com/chanhpng/vbe/issues/880#issuecomment-798421751 for more information.`, http.StatusForbidden)
			return
		}
